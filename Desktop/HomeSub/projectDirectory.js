...
function addEntry(fName, fType) {
  if (isValidName(fName)) {
    if (fType == "folder") {
      ...
      table.innerHTML += "<tr class='entryRow'><td class='iconCell'><div class='fileIcon'> </div></td> <td class='entryName' onClick='entryClick(this)'>" + fName + "</td> <td class='renameCell'> <div class='renameButton' onClick='renameFile(this)'>  </div> </td> <td class='editCell'> <div class='editButton' onClick='editFile(this)'>  </div> </td> </tr>";
    }
  }
}
...
function renameFile(button) {
  var fName = button.parentNode.parentNode.cells[1].innerHTML;
  var renameFileForm = document.getElementById("renameFile");
  document.getElementById("newFileName").focus();
  document.getElementById("newFileName").select();
  document.getElementById("oldFileName").value = fName;
  document.getElementById("newFileName").defaultValue = fName;
  openPopup(document.getElementById("renamePopup"));
}
